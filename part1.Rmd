...
knitr::opts_chunk$set(echo = TRUE, fig.width = 4, fig.height = 2, fig.align = "center")
```
## Introduction
...
ggplot(exponentials, aes(x = x)) + xlab("Exponentials: rexp(n = 1000, rate = 0.2)") +
    ...
    geom_vline(aes(xintercept = mean(exponentials$x), color = "Empirical")) +
    scale_color_manual("Mean", values = c(Theoretical="blue", Empirical="red")) +
    stat_function(fun=dnorm, args=list(mean=mean(exponentials$x),
                                       sd=sd(exponentials$x)),size=1,color="red") +
    ggtitle("Exponential distribution", paste(nosim, "random exponentials at rate", lambda))
```

The empirical or sample mean of a relatively large sample aproximately estimates the theoretical or population mean `1/lambda`, which is `r 1/lambda`.
```{r}
mean(exponentials$x)
```

The empirical or sample variance of a relatively large sample aproximately estimates the theoretical or population variance `(1/lambda)^2`, which is `r (1/lambda)^2`..
```{r}
var(exponentials$x)
```
...
Random variables are said to be independent and identically distributed (IID) if they are independent and all are drawn from the same population, therefore, random exponentials are a case of IID variables.
...
This theorem can be visually demonstrated by plotting a histogram of a reasonably large (e.g. 1000) number of averages of a reasonably large (e.g. 40) sample size of random exponentials with a rate parameter $\lambda$ (lambda), for instance, equals to 0.2. Overlaying the histogram and the normal density curve of the data demonstrates the CLT: the  distribution of averages of exponentials is normally distributed even when the population of exponentials is not normally distributed, as shown in the previous plot.
...
g2 <- ggplot(avgs, aes(x = x)) +
    ...
    stat_function(fun = dnorm,
                  args = list(mean = mean(avgs$x), sd = sd(avgs$x)), color = "red", size = 1) +
